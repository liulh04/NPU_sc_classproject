...
if hqyj_mqtt_clt.rc == 0:
  ...
  while True:
    ...
    if ('soilhum' in mqtt_data) and ('id' in mqtt_data) and (mqtt_data['id'] == 0):
      print('Soil humidity: ')
      print((float(mqtt_data["soilhum"])))
    if ('soiltem' in mqtt_data) and ('id' in mqtt_data) and (mqtt_data['id'] == 0):
      print('Soil temperature: ')
      print((float(mqtt_data["soiltem"])))
