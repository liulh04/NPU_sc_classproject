...
if hqyj_mqtt_clt.rc == 0:
  ...
  while True:
    ...
    if ('light' in mqtt_data) and ('id' in mqtt_data) and (mqtt_data['id'] == 0):
      if (float(mqtt_data["light"])) > 30000:
        hqyj_mqtt_clt.client.publish('APP2AIOTSIM', payload=json.dumps(({"sunshade":False, "id":0}),ensure_ascii=False))
      if (float(mqtt_data["light"])) < 30000:
        hqyj_mqtt_clt.client.publish('APP2AIOTSIM', payload=json.dumps(({"sunshade":True, "id":0}),ensure_ascii=False))
