...
class GeneticAlgorithm:
    def __init__(self, pop_size=100, max_gen=500, elite_size=5):
    ...
    def initialize_population(self):
        ...
        # 添加一些启发式生成的初始解
        self.population.extend(self.generate_heuristic_solutions())
    
    def generate_heuristic_solutions(self):
        # 生成一些启发式初始解
        solutions = []
        
        # 按总加工时间升序排列
        total_time = [sum(pt) for pt in processing_time]
        solutions.append(sorted(range(1, n_jobs+1), key=lambda x: total_time[x-1]))
        
        # 按总加工时间降序排列
        solutions.append(sorted(range(1, n_jobs+1), key=lambda x: -total_time[x-1]))
        
        # 按机器1加工时间升序排列
        solutions.append(sorted(range(1, n_jobs+1), key=lambda x: processing_time[x-1][0]))
    ...
    def mutation(self, individual, mutation_rate=0.3):
        # 多种变异操作
    ...
    def local_search(self, individual):
        # 简单的2-opt局部搜索
        best_cost = self.evaluate(individual)
        improved = True
        ...
        while improved:
            ...
            for i in range(len(individual)-1):
                for j in range(i+1, len(individual)):
                    ...
                    new_cost = self.evaluate(new_individual)
    ...
    def run(self):
        ...
        for gen in range(self.max_gen):
            ...
            if min_cost < best_cost:
                best_cost = min_cost
                best_sequence = self.population[costs.index(min_cost)].copy()
                stagnation_count = 0
            else:
                stagnation_count += 1
            
            # 自适应变异率
            mutation_rate = 0.3 if stagnation_count > 20 else 0.2
            ...
            for i in range(0, len(selected), 2):
                if i+1 < len(selected) and random.random() < 0.9:  # 高交叉概率
            ...
            for i in range(len(offspring)):
                offspring[i] = self.mutation(offspring[i], mutation_rate)
            
            # 局部搜索 - 对部分优秀个体进行
            if gen % 10 == 0 and gen > 0:
                for i in range(min(5, len(offspring))):
            ...
            if gen % 20 == 0 or gen == self.max_gen-1:
                print(f"Generation {gen}: Best Cost = {best_cost:.2f}, Avg Cost = {avg_cost:.2f}")
            
            # 提前终止条件
            if stagnation_count > 50 and gen > self.max_gen//2:
                print(f"Early stopping at generation {gen} due to stagnation")
                break
        
        # 最终局部搜索
        best_sequence = self.local_search(best_sequence)
        best_cost = self.evaluate(best_sequence)
...
if __name__ == "__main__":
    # 运行遗传算法求解最优序列
    ga = GeneticAlgorithm(pop_size=100, max_gen=500)
